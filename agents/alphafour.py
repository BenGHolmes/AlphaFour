        ...
        for i in range(7):
            if col_has_move[i]:
                if self.state[2].all():
                    ...
                    self.children[i] = Node(self.state + np.array([moves[move_idx], np.zeros((6,7)), -np.ones((6,7))]))
                else:
                    # If player 2's turn, add move to state[1], and add 1 to state[2]
                    self.children[i] = Node(self.state + np.array([np.zeros((6,7)), moves[move_idx], np.ones((6,7))]))
                move_idx += 1                
...
class AlphaFour(Agent):
    ...
    def select(self, node):
        ...
        while node.children is not None:
            ...
            ucb[[True if c is None else False for c in node.children]] = -np.inf  # Avoid invalid moves
    ...
    def get_game_state(game_board):
        ...
        return np.array([np.where(game_board == 1, 1, 0), np.where(game_board == -1, 1, 0), np.ones((6,7))])
